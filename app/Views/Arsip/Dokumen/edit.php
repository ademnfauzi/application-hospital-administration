...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
             <h1><b> EDIT DATA ARSIP DOKUMEN </b></h1> 
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="<?= base_url('Arsip_dokumen/index') ?>">Data Arsip Dokumen</a></li>
              <li class="breadcrumb-item active">Edit Data Arsip Dokumen</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    <section class="content">
                  ...
                  <?php } ?>
    <form action="<?= base_url('Arsip_dokumen/update') .'/'. $dokumen['id_arsip_dokumen']; ?>" method="post" class="row g-3 needs-validation" novalidate enctype="multipart/form-data">
    <input type="hidden" name="id_arsip_dokumen" value="<?= $dokumen['id_arsip_dokumen']; ?>">
              ...
              <div class="card-body">
                <label for="jabatan" style="font-size: 20px;" class="form-label">Divisi*</label>
                  <select class="form-select" id="jabatan" required name="divisi">
                    ...
                    <option value="1" <?= ($dokumen['id_divisi'] == '1') ? 'selected' : ''?>>Badan Pengurus Harian</option>
                    <option value="2" <?= ($dokumen['id_divisi'] == '2') ? 'selected' : ''?>>Penelitian Dan Pengembangan</option>
                    <option value="3" <?= ($dokumen['id_divisi'] == '3') ? 'selected' : ''?>>Minat Dan Bakat</option>
                    <option value="4" <?= ($dokumen['id_divisi'] == '4') ? 'selected' : ''?>>Media Kreatif</option>
                    <option value="5" <?= ($dokumen['id_divisi'] == '5') ? 'selected' : ''?>>Humas Internal</option>
                    <option value="6" <?= ($dokumen['id_divisi'] == '6') ? 'selected' : ''?>>Humas Eksternal</option>
                    <option value="7" <?= ($dokumen['id_divisi'] == '7') ? 'selected' : ''?>>Dana Usaha</option>
                  </select>
                  <div class="invalid-feedback">
                    Please select a valid division.
                  </div>
